use sea_orm::{prelude::*,QueryOrder,ConnectionTrait,TransactionTrait,ActiveValue,ActiveModelTrait,TransactionError};
// use crate::error::Error;
use crate::forms::{UserSearchForm,CreateUserForm};
use crate::{Paged,SearchUserResult};
use crate::entity as entity;
use chrono::{Utc};
...
pub async fn create_user(database: &(impl ConnectionTrait + TransactionTrait),form: CreateUserForm) -> Result<entity::user::Model,DbErr> {
    let mut active_model: entity::user::ActiveModel = <entity::user::ActiveModel as ActiveModelTrait>::default();
    active_model.name = ActiveValue::Set(form.data().name().to_string());
    entity::user::Entity::insert(active_model).exec_with_returning(database).await

}
pub async fn create_password_for_user(database:&(impl ConnectionTrait + TransactionTrait),user_id: entity::user::Id, hashed_password:String) -> Result<entity::user_passwords::Model,DbErr> {
    let mut active_model: entity::user_passwords::ActiveModel = <entity::user_passwords::ActiveModel as ActiveModelTrait>::default();
    ...
    entity::user_passwords::Entity::insert(active_model).exec_with_returning(database).await    
}


pub async fn create_user_and_password(database: &DatabaseConnection,form:CreateUserForm,hashed_password:String) -> Result<(entity::user::Model, entity::user_passwords::Model),TransactionError<DbErr>> {
    database
        .transaction::<_, (entity::user::Model, entity::user_passwords::Model), sea_orm::DbErr>(|txn| {
            Box::pin(async move {
                // hello
                let user: entity::user::Model = create_user(txn,form.clone()).await?;
                let user_password: entity::user_passwords::Model = create_password_for_user(txn,user.id(),hashed_password).await?;
